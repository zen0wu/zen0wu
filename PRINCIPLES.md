...
- Simplicity wins: optimize for fewer and less intertwined concepts, not less effort.
...
- Make illegal state irrepresentable, but also don't fall into the trap of purism (FP, OO, ...).
...
- No fallback: You have the tendency to add fallback/make things safe. Just let it fail and only add try/catch when absolutely needed.
